 ...
 */
public class M21A06B1 extends Mouse {
    ...
    private final HashMap<Pair<Integer, Integer>, Grid> mapaBombas;
    ...
    public int move(Grid currentGrid, Cheese cheese) {
        ...
        if (!BombaPuesta && numBombas > 0) {
            if (esCruce(currentGrid) || probabilidadCasilla(currentGrid)) {
                BombaPuesta = true;
                numBombas--;
                return Mouse.BOMB;
            } else {
                BombaPuesta = false;
                if (celdasVisitadas.containsKey(new Pair<>(cheese.getX(), cheese.getY()))) { //SOLO CONSTRUIRE CAMINO AL QUESO SI SE ENCUENTRA EN EL MAPA
